...
class Barzahlen extends PaymentMethod {
  ...
  public function preparePaymentProcess($order) {
    ...
    try {
      ...
      writeLog(self::LOGFILE, $e, 1);
    }

    if($payment->isValid()) {
      $smarty->assign('paymentSlipLink', $payment->getPaymentSlipLink());
      $smarty->assign('expirationNotice', mb_convert_encoding($payment->getExpirationNotice(), 'iso-8859-15', 'utf-8'));
      $smarty->assign('infotext1', mb_convert_encoding($payment->getInfotext1(), 'iso-8859-15', 'utf-8'));
      $smarty->assign('infotext2', mb_convert_encoding($payment->getInfotext2(), 'iso-8859-15', 'utf-8'));
  ...
  public function verifyNotification($order, $paymentHash, $args) {
    ...
    $shopId = $oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_shopid"];
    $notificationKey = $oPlugin->oPluginEinstellungAssoc_arr[$this->cModulId . "_notificationkey"];

    $notification = new Barzahlen_Notification($shopId, $notificationKey, $args);
    try {
  ...
  protected function _checkOrderDetails($order, $args) {
    ...
    if(number_format($order->fGesamtsumme, 2, '.', '') != $args['amount']) {
      writeLog(self::LOGFILE, 'IPN: Amount not valid - ' . serialize($args), 1);
      return false;
    }

    if($order->cStatus != BESTELLUNG_STATUS_OFFEN && $order->cStatus != BESTELLUNG_STATUS_IN_BEARBEITUNG) {
      writeLog(self::LOGFILE, 'IPN: Order already handled', 1);
      return false;
    }
    ...
    if($currency->cISO != $args['currency']) {
      writeLog(self::LOGFILE, 'IPN: Currency not valid - ' . serialize($args), 1);
      return false;
    }
   ...
   *
   * @global object $oPlugin payment plugin
  ...
  protected function _updatePaymentState($kBestellung, $state) {
    ...
    $newState = 'Barzahlen ('.$oPlugin->oPluginSprachvariableAssoc_arr[$state].')';
